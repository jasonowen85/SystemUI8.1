...
public class VolumeDialogControllerImpl implements VolumeDialogController, Dumpable {
    ...
    public void destroy() {
        if (D.BUG) Log.d(TAG, "sxf destroy");
    ...
    private boolean shouldShowUI(int flags) {
        updateStatusBar();
        Log.i(TAG,"sxf  mStatusBar = "+(mStatusBar != null)+ " "+(mStatusBar.getWakefulnessState() != WakefulnessLifecycle.WAKEFULNESS_ASLEEP)
        +"   "+(mStatusBar.getWakefulnessState() != WakefulnessLifecycle.WAKEFULNESS_GOING_TO_SLEEP)
        +"   "+mStatusBar.isDeviceInteractive() +"      "+((flags & AudioManager.FLAG_SHOW_UI) != 0));
...
//                && mStatusBar.isDeviceInteractive()
    ...
    boolean onVolumeChangedW(int stream, int flags) {
        ...
        boolean changed = false;
        Log.i(TAG, "sxf onVolumeChangedW: showUI = "+showUI+" fromKey = "+fromKey+" showVibrateHint = "
                +showVibrateHint+"  showSilentHint = "+showSilentHint);
        if (showUI) {
            changed |= updateActiveStreamW(stream);
        }
        Log.i(TAG, "sxf onVolumeChangedW: changed = "+changed);
    ...
    private final class VC extends IVolumeController.Stub {
        ...
        public void volumeChanged(int streamType, int flags) throws RemoteException {
             Log.d(TAG, " sxf volumeChanged " + AudioSystem.streamToString(streamType)
                    + " " + Util.audioManagerFlagsToString(flags));
            Log.d(TAG,"sxf mDestroyed = "+mDestroyed);
            if (mDestroyed) return;
