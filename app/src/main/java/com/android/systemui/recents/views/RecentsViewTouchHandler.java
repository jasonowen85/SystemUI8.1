...
public static final boolean DEBUG = false;
    public static final String TAG = "RecentsViewTouchHandler";
    ...
    void cancelBackActionButtonClick() {
        mRv.getBackActionButton().setPressed(false);
    }
    ...
    private boolean isWithinBackActionButton(float x, float y) {
        Rect rect = mRv.getStackBackButtonBoundsFromStackLayout();
        return mRv.getBackActionButton().getVisibility() == View.VISIBLE &&
                mRv.getBackActionButton().pointInView(x - rect.left, y - rect.top, 0 /* slop */);
    }
    ...
    private void changeBackActionButtonDrawableHotspot(float x, float y) {
        Rect rect = mRv.getStackBackButtonBoundsFromStackLayout();
        mRv.getBackActionButton().drawableHotspotChanged(x - rect.left, y - rect.top);
    }
    ...
    private boolean handleTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isWithinBackActionButton(evX, evY)) {
                    changeBackActionButtonDrawableHotspot(evX, evY);
                    mRv.getBackActionButton().setPressed(true);
                }
                break;
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mRv.getBackActionButton().isPressed() && isWithinBackActionButton(evX, evY)) {
                    changeBackActionButtonDrawableHotspot(evX, evY);
                }
                ...
                if (mDragRequested) {
                    ...
                    if (mIsDragging) {
                        ...
                        //Reaon: 屏蔽分屏事件响应
                        /*
                        // 长按TaskView时触发，Dock展示虚拟大区域，准备接纳
