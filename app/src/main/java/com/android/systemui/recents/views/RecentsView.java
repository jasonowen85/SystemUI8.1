...
public class RecentsView extends FrameLayout {
    ...
    private TextView mCloseTaskButton;
    private TextView mRecentBackButton;
    ...
    private final float mBackButtonShadowRadius;
    private final PointF mBackButtonShadowDistance;
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        //added by sxf
        if (null != mCloseTaskButton) {
            removeView(mCloseTaskButton);
        }
        mCloseTaskButton = (TextView) inflater.inflate(R.layout.recents_stack_close_button, this, false);
        addView(mCloseTaskButton);

        //add back bt
        if(null != mRecentBackButton){
            removeView(mRecentBackButton);
        }
        mRecentBackButton = (TextView) inflater.inflate(R.layout.recents_stack_back_button, this, false);
        mBackButtonShadowRadius = mRecentBackButton.getShadowRadius();
        mBackButtonShadowDistance = new PointF(mRecentBackButton.getShadowDx(),
                mRecentBackButton.getShadowDy());
        addView(mRecentBackButton);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            //added by sxf
            measureChild(mCloseTaskButton,
                    MeasureSpec.makeMeasureSpec(width, MeasureSpec.AT_MOST),
                    MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));

            measureChild(mRecentBackButton,
                    MeasureSpec.makeMeasureSpec(width, MeasureSpec.AT_MOST),
                    MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            //added by sxf
            int childWidth = mCloseTaskButton.getMeasuredWidth();
            mCloseTaskButton.layout(left,buttonBounds.top,left+childWidth,buttonBounds.bottom);

            Rect backRect = getStackBackButtonBoundsFromStackLayout();
            mRecentBackButton.layout(backRect.left, backRect.top, backRect.right,
                    backRect.bottom);
        }
    ...
    public final void onBusEvent(DragStartEvent event) {
        //接收长按TaskView时发出的event,显示Dock可见区域
...
//        }
        ...
        //长按TaskView，显示下滑关闭按钮
        if(null != event){
            Point point = event.tlOffset;
            Task task = event.task;
            TaskView taskView = event.taskView;
            Rect rect = getTaskRect(taskView);
            Log.d(TAG,"point: x = " + point.x + "  y = " + point.y);
            Log.d(TAG,"task: " + task.toString());
            Log.d(TAG,"rect: " + rect.toString());
            int btLeft = (rect.left + rect.right - mCloseTaskButton.getMeasuredWidth()) / 2;
            mCloseTaskButton.layout(btLeft,rect.bottom + 15,btLeft + mCloseTaskButton.getMeasuredWidth(),rect.bottom + 15 + mCloseTaskButton.getMeasuredHeight());
            mCloseTaskButton.setVisibility(VISIBLE);
        }
    }
...
//Kotei Modefy Start 2018/08/27 sxf
//Reaon: 屏蔽分屏事件			
            //触发分屏
...
//                        event.taskView));
//            }
//Kotei Modefy End 2018/08/27 sxf
        } else {
        ...
        //added by sxf
        if(null != mCloseTaskButton){
            mCloseTaskButton.setVisibility(INVISIBLE);
        }
    }
    ...
    Rect getStackActionButtonBoundsFromStackLayout() {
        ...
        */
        Rect windowRect = Recents.getSystemServices().getWindowRect();
        int buttonLeft = (windowRect.width() - mStackActionButton.getMeasuredWidth()) / 2;
        int buttonTop = windowRect.height() - mStackActionButton.getMeasuredHeight() - mStackActionButton.getPaddingBottom() - 83;
        actionButtonRect.set(buttonLeft, buttonTop, buttonLeft + mStackActionButton.getMeasuredWidth(),windowRect.height() - mStackActionButton.getPaddingBottom());
        //Kotei Modefy End 2018/08/27 sxf

        return actionButtonRect;
    }
	
	    Rect getStackBackButtonBoundsFromStackLayout() {
        Rect actionButtonRect = new Rect();
        Rect windowRect = Recents.getSystemServices().getWindowRect();
        Log.d(TAG, "===> windowRect: " + windowRect.toString());
        int buttonLeft = (windowRect.width() - mRecentBackButton.getMeasuredWidth()) / 2;
        int buttonTop = 30;
        Log.d(TAG, "===> buttonWidth: " + mRecentBackButton.getMeasuredWidth() + "   buttonHeight: " + mRecentBackButton.getMeasuredHeight());
        actionButtonRect.set(buttonLeft, buttonTop, buttonLeft + mRecentBackButton.getMeasuredWidth(), buttonTop + mRecentBackButton.getMeasuredHeight());
        Log.d(TAG, "===> 2 actionButtonRect: " + actionButtonRect.toString());
        return actionButtonRect;
    }
    ...
    View getBackActionButton() {
        return mRecentBackButton;
    }
