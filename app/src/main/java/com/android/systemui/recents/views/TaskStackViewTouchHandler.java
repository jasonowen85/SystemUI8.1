 ...
 */
class TaskStackViewTouchHandler implements RecentsSwipeHelper.Callback {
...
//    SwipeHelper mSwipeHelper;
    RecentsSwipeHelper mSwipeHelper;
    ...
    public TaskStackViewTouchHandler(Context context, TaskStackView sv,
        ...
        mSwipeHelper = new RecentsSwipeHelper(RecentsSwipeHelper.Y, this, context) {
            @Override
    ...
    private boolean handleTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mActiveTaskView = findViewAtPoint(mDownX + mSv.getScrollX(), mDownY);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                mVelocityTracker.addMovement(ev);
                if (canMoveDis(deltaX) && xDiff > mScrollTouchSlop && xDiff > yDiff) {
                    mIsScrolling = true;
                    // 正向或者负向移动，屏幕跟随手指移动
                    mSv.scrollBy(deltaX, 0);
            ...
            case MotionEvent.ACTION_UP: {
                ...
                int x = (int) ev.getX(activePointerIndex);
                int velocitx = (int) mVelocityTracker.getXVelocity(mActivePointerId);
                if (mIsScrolling && Math.abs(velocitx) > mMinimumVelocity &&
                    ...
                    float maxX = mSv.getTotalTaskWidth() - 1920;
                    mScroller.fling(mDownScrollP, mDownX,mSv.getScrollX(), -velocitx, 0, (int) maxX,
                            mOverscrollSize);
                    mSv.invalidate();
                    if(DEBUG)Log.d(TAG,"===> start fling");
                }
    ...
    private boolean canMoveDis(int deltaX) {
        ...
        // deltaX>0 说明手指向左划
        int leftX = mSv.getTotalTaskWidth() - 1920;
        if (deltaX > 0) {
            if (scrollX >= leftX) {
                return false;
            } else if (scrollX + deltaX > leftX) {
                mSv.scrollTo(leftX, 0);
                return false;
            }
        }
        return true;
    ...
    public View getChildAtPosition(MotionEvent ev) {
        TaskView tv = findViewAtPoint((int) ev.getX() + mSv.getScrollX(), (int) ev.getY());
