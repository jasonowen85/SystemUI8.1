 ...
 *
 * TaskStackView的布局逻辑.此布局需要能够仅在传入信息的情况下计算堆栈布局，而无需特定于活动的上下文.
 * 此布局可以具有聚焦和未聚焦两个状态，并且在聚焦状态下，有一个任务在堆栈中更显着地显示。
 */
public class TaskStackLayoutAlgorithm {
    ...
    public static class VisibilityReport {
        ...
        public int numVisibleThumbnails;//缩略图
    ...
    public void initialize(Rect displayRect, Rect windowRect, Rect taskStackBounds,
        ...
        //Reaon:修改任务管理界面一页最多显示7个item,并固定item的高度 
//        mTaskRect.set(0, mStackRect.top, displayRect.width() / 7,  mStackRect.top+400);
        float right = (displayRect.width() - 72 * 5) / 4.5f;
        mTaskRect.set(0, mStackRect.top + 15, (int) right,  mStackRect.top+420);
    ...
    public VisibilityReport computeStackVisibilityReport(ArrayList<Task> tasks) {
        ...
        for (int i = tasks.size() - 1; i >= 0; i--) {
            ...
            if (isFrontMostTaskInGroup) {
                getStackTransform(i,taskProgress, taskProgress, mInitialScrollP, mFocusState,
    ...
    public TaskViewTransform getStackTransform(Task task, float stackScroll, int focusState,
        ...
        if (mFreeformLayoutAlgorithm.isTransformAvailable(task, this)) {
            ...
            //Kotei Modefy Start 2018/08/27 sxf
            //Reaon:为计算TaskView显示坐标，添加View的索引
            int taskIndex = mTaskIndexMap.get(task.key.id);
            getStackTransform(taskIndex,taskProgress, nonOverrideTaskProgress, stackScroll, focusState,
     ...
     */
    public void getStackTransform(int taskIndex, float taskProgress, float nonOverrideTaskProgress,
        ...
        //Kotei Modefy Start 2018/08/27 sxf
        //Reaon:修改为根据View的index值计算View显示的坐标
//        int x = (mStackRect.width() - mTaskRect.width()) / 2;
//        int x = (index++ % 6) * (mTaskRect.width()+ 30);//mStackRect.width()/(7*8);
        int x = taskIndex * (mTaskRect.width()+ 72) + 72;
        //Kotei Modefy End 2018/08/27 sxf
        int y;
        float z;
        float dimAlpha;
        ...
        transformOut.translationZ = 0;//屏蔽层叠效果
        transformOut.dimAlpha = 0f;
        transformOut.viewOutlineAlpha = 0.5f;
        transformOut.rect.set(mTaskRect);
        transformOut.rect.offset(x, 0);
        Utilities.scaleRectAboutCenter(transformOut.rect, transformOut.scale);
        //设置TaskView可见状态
        transformOut.visible = true;//(transformOut.rect.top < mStackRect.bottom) &&
//                (frontTransform == null || transformOut.rect.top != frontTransform.rect.top);
    ...
    public float getDeltaPForX(int downX, int x) {
        if (Recents.getConfiguration().isLowRamDevice) {
            return mTaskStackLowRamLayoutAlgorithm.scrollToPercentageX(downX - x);
        }
    ...
    public int getXForDeltaP(float downScrollP, float p) {
        if (Recents.getConfiguration().isLowRamDevice) {
            return mTaskStackLowRamLayoutAlgorithm.percentageToScrollX(downScrollP - p);
        }
...
//        taskStackBounds.inset((taskStackBounds.width() - targetStackWidth) / 2, 0);
        taskStackBounds.set(0,72,1919,598);
    }
    ...
    private void updateFrontBackTransforms() {
        ...
        //Reaon:为计算TaskView显示坐标，添加View的索引
        int endIndex = 0;
        if(null != mTaskIndexMap && mTaskIndexMap.size() > 0)endIndex = mTaskIndexMap.size()-1;
        getStackTransform(0,min, min, 0f, mFocusState, mBackOfStackTransform, null,
                true /* ignoreSingleTaskCase */, true /* forceUpdate */);
        getStackTransform(endIndex,max, max, 0f, mFocusState, mFrontOfStackTransform, null,
                true /* ignoreSingleTaskCase */, true /* forceUpdate */);
        //Kotei Modefy End 2018/08/27 sxf
