...
public class TaskStackView extends FrameLayout implements TaskStack.TaskStackCallbacks,
    ...
    // We keep track of the task view focused by user interaction and draw a frame around it in the grid layout.
    // 我们跟踪用户交互所关注的任务视图，并在网格布局中围绕它绘制框架。
    ...
    private DropTarget mFreeformWorkspaceDropTarget = new DropTarget() {
        ...
        public boolean acceptsDrop(int x, int y, int width, int height, Rect insets,
            ...
            // This drop target has a fixed bounds and should be checked last, so just fall through if it is the current target
            // 这个放置目标有一个固定的边界，应该在最后检查，所以如果它是当前目标，那么它就会掉落
            if (!isCurrentTarget) {
    ...
    void bindVisibleTaskViews(float targetStackScroll, boolean ignoreTaskOverrides) {
        ...
        for (int i = taskViewCount - 1; i >= 0; i--) {
            ...
            /*
             * 如果在重新布局之前更新堆栈，则延迟的TaskView集可能与堆栈不同.
             * 如果任务视图不再在堆栈中，则只需将其返回到视图池。
             */
   ...
   public void updateLayoutAlgorithm(boolean boundScrollToNewMinMax,
    ...
    public void computeScroll() {
		if (DEBUG) Log.d(TAG, "start computeScroll");
        if (mStackScroller.computeScroll()) {
            ...
            if(DEBUG)Log.d(TAG,"mScroller.getCurrX(): " + mStackScroller.mScroller.getCurrX() + "   mScroller.getCurrY(): " + mStackScroller.mScroller.getCurrY());
            scrollTo(mStackScroller.mScroller.getCurrX(),mStackScroller.mScroller.getCurrY());
            invalidate();
        }
		...
		*/
		if (DEBUG) Log.d(TAG, "end computeScroll");
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        /*
         * 更新稳定堆栈边界，但仅在稳定边界发生变化时才更新当前堆栈边界。
         * 这是因为我们可能会在拖动当前堆栈边界反映目标放置区域的位置时得到虚假度量。
         */
        ...
        //这个方法决定了当前View的大小,即当前TaskStackView的大小
     ...
     * Measures a TaskView.
     */
    private void measureTaskView(final TaskView tv) {
...
//        tv.setClipViewInStack(true);
            ...
            if (Recents.getConfiguration().isLowRamDevice) {
                ...
                if (mStack.getTaskCount() > 0 && !mStackActionButtonVisible
                    ...
                    EventBus.getDefault().send(
                            new ShowStackActionButtonEvent(true ));
                }
                if (DEBUG) Log.d(TAG, "end onStackScrollChanged --- 1");
                return;
            }
    ...
    public int getTotalTaskWidth(){
        if(DEBUG)Log.d(TAG,"count: " + mStack.getTaskCount());
        int width = mStack.getTaskCount() * (346 + 72) + 72;
