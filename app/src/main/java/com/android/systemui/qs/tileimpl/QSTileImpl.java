...
public abstract class QSTileImpl<TState extends State> implements QSTile {
    ...
    public static int getColorForState(Context context, int state) {
        //--------start modified by sxf 修改下拉菜单快捷设置按键不同状态对应的按键颜色-----------------
        switch (state) {
            case Tile.STATE_UNAVAILABLE:
                return Utils.getDefaultColor(context, R.color.qs_tile_state_unavailable);
            case Tile.STATE_INACTIVE:
                return Utils.getDefaultColor(context, R.color.qs_tile_state_inactive);
            case Tile.STATE_ACTIVE:
                return Utils.getDefaultColor(context, R.color.qs_tile_state_active);
            default:
