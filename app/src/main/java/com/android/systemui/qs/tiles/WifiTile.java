...
public class WifiTile extends QSTileImpl<SignalState> {
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        //Kotei Modefy Start 2018/08/27 sxf
        //Reaon:  添加非空判断
        if(null == mSignalCallback || null == mSignalCallback.mInfo){
            return;
        }
...
//            state.slash.isSlashed = true;//去掉图标上的斜杠
            state.state = Tile.STATE_INACTIVE;
//            state.icon = ResourceIcon.get(R.drawable.ic_qs_wifi_disabled);
            state.icon = ResourceIcon.get(R.mipmap.home_pulldown_btn_wifi_off);
            state.label = r.getString(R.string.quick_settings_wifi_label);
        } else if (wifiConnected) {
            state.icon = ResourceIcon.get(cb.wifiSignalIconId);
            state.label = removeDoubleQuotes(cb.enabledDesc);
        } else if (wifiNotConnected) {
            state.icon = ResourceIcon.get(R.mipmap.ic_qs_wifi_disconnected);
            state.label = r.getString(R.string.quick_settings_wifi_label);
        } else {
